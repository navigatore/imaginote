    ...
    for (const auto& row: fields)
    {
        for (const auto& field: row)
        {
            if (field.height > 0)
            {
                ...
                painter->drawText(x + fieldSize / 2, y + fieldSize / 2, QString(std::to_string(field.height).c_str()));
            }
        }
    }
}
...
{
    float radius = static_cast<float>(std::sqrt(pxWidth * pxWidth + pxHeight * pxHeight));
    ...
    pen.setColor(color);
    painter->setPen(pen);
    painter->drawLine(playerPxPosX, playerPxPosY, playerPxPosX + x, playerPxPosY - y);
}
//*********************************************************************************************************************
...
//*********************************************************************************************************************
void MapWidget::setAngleX(const Angle& angleX)
{
    this->angleX = angleX;
}
//*********************************************************************************************************************
